...
for (element in lst){
  if (i%%2 ==0){
    summ <- summ+1 #not ideal for the memory of r code as it will run through every element of the list
    }
}
...
print_professions <- function(dic) {
  for (i in names(dic)){
  print(paste0(i, "was a", dic[[i]],".")) # paste0 is to remove the space before the .
    }
}
print_professions(scientists)
...
# Use ifelse() to classify values as "Positive" or "Negative"
...
mydframe_vectorised <- mydframe + 10 * sin(0.75 * pi)
...
# Exercise 8
mean_for_each_row <- function(df){apply(df,1,mean)}
mean_for_each_row(sample.df)

# Exercise 9
mean_for_each_column <- function(df){apply(df,2,mean)}
mean_for_each_column(sample.df)

# Exercise 10
# Define the function 'fun'
fun <- function(x, char = FALSE) {
  result <- x^2  # Calculate the square of the number
  if (char) {
    result <- as.character(result)  # Convert the result to a character if 'char' is TRUE
  }
  return(result)
}

# Example usage
print(fun(4))         # Output the square of 4
print(fun(4, char = TRUE))  # Output the square of 4 as a character
